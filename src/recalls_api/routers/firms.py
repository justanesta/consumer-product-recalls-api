...
_DESC = (
    "One canonical firm — the cross-source cluster, so a maker appearing under several agencies "
    "(e.g. Honda under NHTSA and USCG) collapses to a single profile. Includes recall counts, the "
    "per-source breakdown (`recalls_by_source`), name variants, and agency registration sidecars: "
    "`firm_usda_attributes` (USDA/FSIS), `firm_uscg_attributes` (USCG boat MIC), and "
    "`firm_fda_attributes` (FDA FEI). Sidecars have DIFFERENT shapes and any may be empty; CPSC "
    "and NHTSA contribute none. `first_recall_at`/`last_recall_at` are null for a firm with no "
...
@router.get(
    ...
    summary="Fetch a canonical (cross-source) firm profile with its agency registration sidecars.",
    description=_DESC,
)
async def get_firm(
