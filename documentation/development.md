...
## Exploring the OpenAPI Spec & Docs Locally

`openapi.json` is **generated by introspection** — FastAPI reads your route signatures and Pydantic
models (including every `Field(description=…)` and `Query(description=…)`) and assembles the spec.
That spec is what renders the public docs site (Starlight + `starlight-openapi` at build time; see
[frontend-api-docs-handoff.md](frontend-api-docs-handoff.md)). The fastest way to *see* this
round-trip — author a description in Python, watch it appear in the docs — is below.

### Live, interactive (needs a read-only DSN in `.env`)

The running server serves three things FastAPI builds for free from the spec:
...
| URL | What it is |
|---|---|
| `http://localhost:8000/openapi.json` | The raw generated spec (same shape as the committed file) |
| `http://localhost:8000/docs` | Swagger UI — a renderer consuming that spec |
| `http://localhost:8000/redoc` | ReDoc — a second renderer of the same spec |

```bash
# Pretty-print the live spec, or pull one parameter out of it:
curl -s http://localhost:8000/openapi.json | jq '.info'
curl -s http://localhost:8000/openapi.json | jq '.paths."/recalls".get.parameters[] | select(.name=="source")'
```

Because `--reload` restarts the process on save, editing a description and refreshing `/docs` shows
the new text immediately — no regeneration step for the live view. (The live server boots the
lifespan, which opens the read-only pool, so it needs a valid `NEON_DATABASE_URL_RO`. The two
DB-free paths below don't.)

### DB-free: the author → regenerate → diff loop

Generating the spec **never touches the database** — `export_openapi.generate()` sets a dummy DSN and
calls `app.openapi()` without opening the pool — so you can run this with no `.env` configured:

```bash
# 1. Write the spec from the current source to the committed openapi.json
uv run python -m recalls_api.export_openapi

# 2. Inspect a single field or parameter (jq path mirrors the JSON structure)
jq '.paths."/products/search".get.parameters[] | select(.name=="upc")' openapi.json
jq '.components.schemas.RecallDetail.properties.product_upcs' openapi.json
```

Now make a one-line edit to a description and watch it flow through. For example, change the `upc`
selector's text in `src/recalls_api/routers/products.py` (the `Query(description=…)` on the `upc`
param), then:

```bash
uv run python -m recalls_api.export_openapi   # regenerate
git diff openapi.json                          # see exactly what changed in the contract
git checkout src/recalls_api/routers/products.py openapi.json   # revert the experiment
```

The same works for response-field descriptions — edit a `Field(description=…)` in
`src/recalls_api/models/` (e.g. `RecallDetail.product_upcs` in `models/recalls.py`) and the diff
appears under `components.schemas`. This is the mechanism that makes per-field documentation
single-homed: author it once in the model/route, and it propagates to `openapi.json` and every
renderer automatically.

### Verify the snapshot is in sync

```bash
uv run python -m recalls_api.export_openapi --check   # exit 1 if committed openapi.json is stale
```

This is the same check CI and the `openapi-drift` pre-commit hook run — see the Quality Gate below
and step 6 of [How to Add an Endpoint](#how-to-add-an-endpoint).

---

## Versioning

`pyproject.toml`'s `[project].version` is the **single source of truth**. `recalls_api.__version__`
(`src/recalls_api/__init__.py`) is read back from installed package metadata via
`importlib.metadata.version("consumer-product-recalls-api")` — it is never hardcoded. From there it
cascades to FastAPI's OpenAPI `info.version` (so `/docs` and `openapi.json`), the `/health` response
body, and the cache-validator ETag.

To bump the version:

```bash
# 1. Edit pyproject.toml [project].version (e.g. 0.1.1 -> 0.1.2)
# 2. Re-sync so the installed package metadata picks up the new version
uv sync
# 3. Regenerate the snapshot — info.version lives in openapi.json, so a bump drifts it
uv run python -m recalls_api.export_openapi
git add pyproject.toml openapi.json
```

`uv sync` is required: `importlib.metadata` reads the **installed** distribution, so without a
re-sync the running app still reports the old version. The contract test and `openapi-drift` hook
fail if you bump the version but forget to regenerate `openapi.json`. `tests/test_smoke.py` asserts
`__version__` equals the metadata version (never a literal), so the version is verified in sync
without needing an edit per bump.
