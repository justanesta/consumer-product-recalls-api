...
> **⚠️ Reconciliation 2 (2026-W26, `fix/announced-index-sort-site`, pipeline ADR 0038 §2026-W26).** The `GET /recalls` feed sort moved from `published_at` to **`event_date = coalesce(announced_at, published_at)`** (announce-recency). Consequently, everything below that shows `published_at_keyset_where`, the bare 2-tuple cursor, or `published_at DESC` for the **list** path is **stale**: the keyset builder is now `date_keyset_where(cursor, expected_kind, date_col, id_col)`, the cursor is a kind-tagged 3-tuple `[kind, value, id]` (`e`=recalls list/event_date, `p`=products/published_at, `r`=rank), and `RecallSummary`/`RecallDetail` gained a non-null `event_date` field. Products keep `published_at` (`p`). Trust the current `src/recalls_api/` source + [`openapi.json`](../../openapi.json) + ADR 0004's W26 amendment.
